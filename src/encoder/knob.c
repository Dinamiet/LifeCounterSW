...
void encoderKnobTurn_Task(void* _)
{
	...
	{
		EventData data = {.Knob = KNOB_1, .Increasing = true};
		Observer_Publish(&eventNotifier, EVENT_KNOB, &data);
		--knob1PendingSteps;
	}
	else if (knob1PendingSteps < 0)
	{
		EventData data = {.Knob = KNOB_1, .Increasing = false};
		Observer_Publish(&eventNotifier, EVENT_KNOB, &data);
		++knob1PendingSteps;
	}
	...
	{
		EventData data = {.Knob = KNOB_2, .Increasing = true};
		Observer_Publish(&eventNotifier, EVENT_KNOB, &data);
		--knob2PendingSteps;
	}
	else if (knob2PendingSteps < 0)
	{
		EventData data = {.Knob = KNOB_2, .Increasing = false};
		Observer_Publish(&eventNotifier, EVENT_KNOB, &data);
		++knob2PendingSteps;
	}
}

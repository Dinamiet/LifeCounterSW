...
void Display_ShowInt(EventSide side, int16_t value)
{
	...
	uint16_t positiveValue = (value >= 0) ? value : -value;

	if (value < 0)
		dispData->Digit[0] = 0x40; // Display minus sign

	for (uint8_t i = 0; i < NUM_DIGITS - (value < 0); i++)
	{
		uint8_t segmentData = lookup[positiveValue % 10];

		// Remove leading zeros, but keep single zero
		dispData->Digit[NUM_DIGITS - 1 - i] = (positiveValue > 0 || i == 0) ? segmentData : 0;
